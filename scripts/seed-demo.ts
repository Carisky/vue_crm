...
const users = [
  ...
  { id: "demo-user-marek", name: "Marek Nowak", email: "marek.demo@tsl.local" },
  {
    ...
    email: "kasia.demo@tsl.local",
  },
  {
    id: "demo-user-piotr",
    name: "Piotr Wiśniewski",
    email: "piotr.demo@tsl.local",
  },
];
...
async function main() {
  ...
  const operations = await prisma.workspaceGroup.create({
    data: {
      ...
      color: "#2563eb",
      members: {
        create: users.slice(0, 3).map((user) => ({ userId: user.id })),
      },
    },
  });
  const sales = await prisma.workspaceGroup.create({
    data: {
      ...
      color: "#f59e0b",
      members: {
        create: [users[0], users[3]].map((user) => ({ userId: user.id })),
      },
    },
  });
  ...
  const taskRows = [
    [
      "demo-task-01",
      "Zebrać prognozy wolumenu od klientów",
      "DONE",
      "HIGH",
      -6,
      users[3].id,
    ...
    [
      "demo-task-02",
      "Potwierdzić dostępność przewoźników",
      "IN_REVIEW",
      "REAL_TIME",
      1,
      users[1].id,
    ...
    [
      "demo-task-04",
      "Zweryfikować komplet dokumentów CMR",
      "TODO",
      "MEDIUM",
      5,
      users[2].id,
    ...
    [
      "demo-task-05",
      "Przygotować raport SLA dla klienta",
      "BACKLOG",
      "LOW",
      9,
      users[3].id,
    ...
    [
      "demo-task-06",
      "Wyjaśnić opóźnienie trasy Katowice–Berlin",
      "IN_PROGRESS",
      "REAL_TIME",
      -1,
      users[1].id,
    ...
    [
      "demo-task-07",
      "Uruchomić komunikację statusową",
      "TODO",
      "HIGH",
      2,
      users[0].id,
    ...
    [
      "demo-task-08",
      "Przetestować ścieżkę eskalacji",
      "BACKLOG",
      "MEDIUM",
      6,
      users[2].id,
  ...
  for (const [
    ...
    await prisma.task.create({
      data: {
        ...
        actualHours:
          status === "DONE" ? 5.5 : status === "IN_PROGRESS" ? 3 : null,
        ...
        creatorId: users[0].id,
        projectId:
          id === "demo-task-05" ? onboardingProject.id : mainProject.id,
        parentId,
        assigneeId,
        assigneeGroupId,
      },
    ...
    data: [
      {
        ...
        workspaceId: workspace.id,
        authorId: users[1].id,
        body: "Otrzymaliśmy potwierdzenie od 8 z 10 przewoźników. Dwa tematy wymagają decyzji do jutra.",
      },
      {
        id: "demo-comment-02",
        taskId: "demo-task-02",
        workspaceId: workspace.id,
        authorId: users[0].id,
        body: "Priorytet: zabezpieczyć trasę Katowice–Berlin. Pozostałe możemy zamknąć po południu.",
      },
      {
        id: "demo-comment-03",
        taskId: "demo-task-06",
        workspaceId: workspace.id,
        authorId: users[2].id,
        body: "Dokumenty są kompletne. Czekamy na potwierdzenie nowego ETA od przewoźnika.",
      },
    ],
  });
    ...
    data: [
      {
        ...
        body: "Cel: bezpieczna obsługa zwiększonego wolumenu.\nWłaściciel: Anna Kowalska.\nMiernik: minimum 97% dostaw w SLA.",
        isLocked: true,
      },
      {
        id: "demo-doc-02",
        ...
        authorId: users[1].id,
        sectionId: section.id,
        title: "Ścieżka eskalacji",
        body: "1. Dyspozytor aktualizuje zadanie.\n2. Lider operacji ocenia wpływ.\n3. Przy ryzyku SLA informujemy klienta i managera.",
        isLocked: false,
      },
      {
        id: "demo-doc-03",
        ...
        projectId: mainProject.id,
        authorId: users[2].id,
        title: "Checklista zamknięcia zlecenia",
        body: "CMR, potwierdzenie dostawy, rozliczenie kosztów, informacja dla klienta.",
        isLocked: false,
      },
    ],
  });
    ...
    data: [
      {
        ...
        senderId: users[1].id,
        body: "Dwie trasy wymagają potwierdzenia przewoźnika.",
      },
      {
        id: "demo-message-02",
        conversationId: groupConversation.id,
        senderId: users[0].id,
        body: "Skupmy się najpierw na Berlinie. Decyzję zapisuję też w zadaniu.",
      },
      {
        id: "demo-message-03",
        conversationId: groupConversation.id,
        senderId: users[2].id,
        body: "Jasne, aktualizuję ETA i dokumenty.",
      },
    ],
  });
    ...
    data: [
      {
        ...
        type: "TASK_COMMENT",
        message:
          "Nowy komentarz w zadaniu: Potwierdzić dostępność przewoźników",
      },
      {
        ...
        message: "Zadanie wymaga uwagi: opóźniony termin",
      },
    ],
  });
