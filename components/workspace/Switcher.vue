...
watch(
  [() => route.params["workspaceId"], () => route.query["workspace_id"], data],
  ([routeId, queryId, data]) => {
    const workspaceId =
      (typeof routeId === "string" && routeId
        ? routeId
        : typeof queryId === "string" && queryId
          ? queryId
          : (lastWorkspaceId.value ?? data?.[0]?.$id)) ?? "";
    selectedWorkspace.value =
      data?.find(({ $id }) => $id === workspaceId) ?? "";
    if (selectedWorkspace.value) setWorkspaceId(selectedWorkspace.value.$id);
  },
  { immediate: true },
);
...
<template>
  <div
    ...
    @click="workspaceSelectOpen = false"
  >
    <div class="flex items-center justify-between text-sidebar-foreground/85">
      ...
      >
        <Icon
          ...
          size="20px"
          class="size-5 cursor-pointer text-sidebar-foreground/85 transition hover:opacity-75"
        />
      </button>
    </div>
    <Select
    ...
    >
      <button
        ...
        ref="trigger"
        class="h-12 w-full rounded-md border border-border bg-card pl-3 text-left font-medium text-card-foreground focus-visible:border-primary focus-visible:ring focus-visible:ring-primary/50 focus-visible:ring-offset-0"
      >
        <div
          v-if="selectedWorkspace"
          class="flex items-center justify-start gap-3 font-medium text-card-foreground"
        >
          <WorkspaceAvatar
