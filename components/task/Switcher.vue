...
const {
  ...
  assigneeId,
  mine = false,
} = defineProps<{ projectId?: string; assigneeId?: string; mine?: boolean }>();
...
const { value: view, setQueryValue: setView } = useUrlQuery("tab");
...
const fetchTasks = async () => {
  ...
  const assignee_id =
    assigneeId ?? (filterValues.value["assignee_id"] as string);
  const group_id = filterValues.value["group_id"] as string;
  ...
  if (mine) searchParams.set("mine", "1");
  else if (assignee_id) searchParams.set("assignee_id", assignee_id);
  ...
  requestFetch<{ tasks: FilteredTask[] }>(
    `/api/tasks/filter?${searchParams.toString()}`,
  )
...
const matchesActiveFilters = (task: FilteredTask) => {
  ...
  const activeProject =
    projectId ?? (filterValues.value["project_id"] as string | undefined);
  if (activeProject && task.project_id !== activeProject) return false;

  const activeAssignee =
    assigneeId ?? (filterValues.value["assignee_id"] as string | undefined);
  if (activeAssignee && task.assignee_id !== activeAssignee) {
    if (!mine || !task.assignee_group?.member_ids.includes(activeAssignee))
      return false;
  }

  const activeGroup = filterValues.value["group_id"] as string | undefined;
  if (activeGroup && task.assignee_group_id !== activeGroup) return false;
  ...
  const activeStartDate = filterValues.value["started_at"] as
    | string
    | undefined;
  if (activeDueDate) {
  ...
  if (
    activeSearch &&
    !task.name.toLowerCase().includes(activeSearch.toLowerCase())
  )
    return false;
...
<template>
  <Tabs
    ...
    @update:model-value="handleSetView"
    class="w-full flex-1 rounded-lg border"
  >
    <div class="flex h-full flex-col overflow-auto p-4">
      <div
        class="flex flex-col items-center justify-between gap-y-2 lg:flex-row"
      >
      ...
      <div
        v-if="isLoadingTasks"
        class="flex h-52 w-full flex-col items-center justify-center rounded-lg border"
      >
        <Icon
      ...
      <TabsContent value="table" class="mt-0">
        <TaskDataTable
          v-if="tasks && !isLoadingTasks"
          :columns="columns"
          :data="tasks"
          :on-row-click="handleRowClick"
          :column-visibility-key="columnVisibilityKey"
          :column-visibility-workspace-id="workspaceId"
          :column-sizing-key="columnSizingKey"
