...
const initialCalendarDate = events[0]?.start
  ...
  new Promise((resolve) => {
    calendar = new Calendar(elm as HTMLElement, {
      ...
      eventContent: function (arg) {
        const { id, title, start, extendedProps } =
          arg.event as unknown as Event;
        return {
      ...
      eventClick: function (eventClickInfo) {
        navigateTo(
          `/workspaces/${route.params["workspaceId"]}/tasks/${eventClickInfo.event.id}`,
        );
      },
      datesSet: function (e) {
