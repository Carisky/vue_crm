...
const { isPending, mutate } = useMutation({
    ...
    onError: (error: any) =>
        toast.error(error?.data?.statusMessage ?? t('auth.resetRequestFailed')),
})
        ...
        <CardContent class="p-7">
            <div v-if="sent" class="space-y-4 text-center">
                ...
                <Button variant="secondary" class="w-full" :as-child="true">
                    <NuxtLink href="/sign-in">{{ t('auth.backToSignIn') }}</NuxtLink>
                </Button>
            </div>
            <form v-else class="space-y-4" @submit="handleSubmit">
                <fieldset :disabled="isPending" class="space-y-4">
                    <FormField v-slot="{ componentField }" name="email">
                        <FormItem>
                            <FormControl>
                                <Input type="email" autocomplete="email" :placeholder="t('auth.emailPlaceholder')"
